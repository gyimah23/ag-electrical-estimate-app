...
const ShareOptions: React.FC<ShareOptionsProps> = ({
  ...
  const shareViaWhatsApp = () => {
    ...
    estimateData.items.forEach((item, index) => {
      messageText += `${index + 1}. ${item.name}${item.brand ? ` (${item.brand})` : ''} - ${item.quantity} ${item.unit} x ${currencySymbol}${item.price.toFixed(2)} = ${currencySymbol}${(item.quantity * item.price).toFixed(2)}\n`;
    });
  ...
  const shareViaEmail = () => {
    ...
    body += `MATERIALS LIST:\n`;
    estimateData.items.forEach((item, index) => {
      body += `${index + 1}. ${item.name}${item.brand ? ` (${item.brand})` : ''} - ${item.quantity} ${item.unit} x ${currencySymbol}${item.price.toFixed(2)} = ${currencySymbol}${(item.quantity * item.price).toFixed(2)}\n`;
    });
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button className="bg-blue-600 hover:bg-blue-700 shadow-md hover:shadow-lg transition-all duration-300 w-full">
            Share Estimate
          </Button>
        </motion.div>
      </PopoverTrigger>
      <PopoverContent className="w-96 p-5 shadow-lg border-slate-200">
        <motion.div 
          className="space-y-4"
        ...
        >
          <h4 className="font-medium text-slate-800 text-lg">Share with Client</h4>
          <div className="space-y-2">
            <Label htmlFor="clientName" className="text-sm text-slate-600">Client Name</Label>
            <Input
              id="clientName"
              name="clientName"
              ...
              className="transition-all focus-within:border-blue-400"
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="clientEmail" className="text-sm text-slate-600">Client Email</Label>
            <Input
              id="clientEmail"
              name="clientEmail"
              type="email"
